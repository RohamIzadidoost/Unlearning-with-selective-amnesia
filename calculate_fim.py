...
def parse_args_and_ckpt(device):
    ...
    # Try loading with weights_only=False (needed for checkpoints containing objects like argparse.Namespace)
    try:
        ...
        # If failure is due to safe globals (e.g., argparse.Namespace), try allowlisting it
        if "Unsupported global: GLOBAL argparse.Namespace" in str(e):
...
if __name__ == "__main__":
    ...
    args, ckpt = parse_args_and_ckpt(device)
    config = ckpt['config']
